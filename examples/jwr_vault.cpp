/// \file jwr_vault.cpp
/// \brief Example: JWT-like wrapper ("JWR") for vault body (PBKDF2(+pepper) + AES-256-GCM).
/// \details
///   - Token = base64url(header) "." base64url(body)
///   - Header is informational; body is JSON with KDF/AEAD params + ciphertext
///   - Payload = {"email","password"} encrypted with AES-GCM
...
struct VaultFile {
    ...
    hmac_cpp::secure_buffer<uint8_t, true> aad; // optional
};
...
create_token(const hmac_cpp::secret_string& master,
    ...
    json header = { {"typ","JWR"}, {"alg","AES-256-GCM"}, {"kdf","PBKDF2-HMAC-SHA256"} };
...
bool read_vault(const std::string& path,
    ...
    hmac_cpp::secure_zero(&read_token[0], read_token.size());
...
/// \brief CLI:
/// jwr_vault <path> <email> <passphrase>
